...
def main(train_path, valid_path, test_path, pred_path):
    ...
    """
    pred_path_c = pred_path.replace(WILDCARD, 'c_GDA')
    pred_path_d = pred_path.replace(WILDCARD, 'd_GDA')
    pred_path_e = pred_path.replace(WILDCARD, 'e_GDA')

    # *** START CODE HERE ***
    # Part (c): Train and test on true labels
    x_train, t_train = util.load_dataset(train_path, label_col='t', add_intercept=False)
    x_test, t_test = util.load_dataset(test_path, label_col='t', add_intercept=True)

    model_t = GDA()
    model_t.fit(x_train, t_train)

    util.plot(x_test, t_test, model_t.theta, 'output/p02c_GDA.png')
    ...
    util.plot(x_test, y_test, model_y.theta, 'output/p02d_GDA.png')
    ...
    util.plot(x_test, t_test, model_y.theta, 'output/p02e_GDA.png', correction)
